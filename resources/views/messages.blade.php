...
<x-layout :title="'Messages'" :community="$community" :communities="$communities">
                    ...
                    <div id="message-scroll" class="flex-1 overflow-y-auto bg-white px-2 space-y-2">
                        ...
                        @foreach ($messages->reverse() as $message)
                            <div class="flex {{ $message->user_id === $userId ? 'justify-end' : 'justify-start' }} group items-start gap-2"
                                ...
                                @if ($message->user_id !== $userId)
                                    <div
                                        class="w-9 h-9 rounded-full ml-3 self-end mb-1 flex items-center justify-center overflow-hidden bg-gradient-to-br from-sky-300 to-indigo-300 relative z-[50]">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Avatar markup aligned with Blade
        ...
        function subscribeToActiveConversation() {
            ...
            if (__currentEchoChannelName === channelName) return;
            ...
            try {
                ...
                ch.listen('MessageSent', (e) => {
                    ...
                    if (scrollContainer.querySelector(`[data-message-id="${e.id}"]`)) return;
                    ...
                    try {
                        ...
                        wrapper.innerHTML = `
                            ...
                            <div class="max-w-[75%] flex flex-col items-start">
                                ...
                                <div class="relative px-4 py-2 max-w-[255px] break-words text-sm bg-gray-200 text-gray-900 rounded-[15px] self-start shadow-sm hover:scale-[1.02] transition-transform z-[2]">
                                    ${e.body.replace(/\n/g, '<br>')}
                                ...
                                <div class="text-[9px] text-gray-400 text-left mb-2">
                ...
                if (newList && container) {
                    ...
                    try {
                        ...
                        const matched = items.find(a =>
                            (wantedChannel && a.href.includes(`channel_id=${wantedChannel}`)) ||
                            (wantedThread && a.href.includes(`thread_id=${wantedThread}`))
                        );
